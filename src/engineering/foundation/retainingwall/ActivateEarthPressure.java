...
public class ActivateEarthPressure {
    private final double Sw; //土壤單位重(tf/m^3）
    private final double Sc; //土壤凝聚力（tf/m^2）
    private double wallH; //牆之垂直高度（m）
    private double pointPD; //牆頂地表面至欲求土壓力點之垂直深度（ｍ）
    private final double Sf; //牆背土壤之內摩擦角（度）通常採用 牆體若為木材、鋼材或預鑄鋼筋混凝土，則因牆面較平滑，最大牆面摩擦角可採用牆體若為場鑄鋼筋混凝土，其牆面較粗糙，最大牆面摩擦角可採用
    private final double wallWithSf; //牆背面與土壤間之摩擦角（度）
    private final double backSDeg; //牆背地表面與水平面之交角（度）
    private final double backDeg; //牆背面與垂直面交角（度），以逆時針方向為正，順時針方向為負
